...
object LocalModelManager {
    ...
    val AVAILABLE_MODELS = listOf(
        // ── Modelos por defecto (Google Gemma 4, censurados pero estables) ─────
        ModelInfo(
            id = "gemma4-e2b",
            displayName = "Gemma 4 E2B — 2.6 GB (recomendado)",
        ...
        ModelInfo(
            id = "gemma4-e4b",
            displayName = "Gemma 4 E4B — 3.6 GB (más potente)",
    ...
    fun customModel(): ModelInfo? {
        val url = com.blackclaw.android.utils.KVUtils.getCustomLocalModelUrl()
    ...
    interface DownloadCallback {
        ...
        /** Called periodically to check if the download should be cancelled.
         *  Return true to abort the download cleanly. */
    ...
    private fun canWriteToDirectory(dir: File): Boolean {
        val probe = File(dir, ".blackclaw-write-probe")
        return runCatching {
    ...
    fun downloadModel(
        ...
        try {
                ...
                outputStream.use { output ->
                    while (true) {
                        ...
                        if (callback.isCancelled()) {
                            XLog.i(TAG, "Download cancelled by caller at $downloadedBytes bytes")
            ...
            if (!tempFile.renameTo(targetFile)) {
                callback.onError("Download finished but BlackClaw could not move the model into place")
                return
            }
