 ...
 * - BlackClaw original: screen diff (previousScreenTexts)
 *
 * Recovery is 3-level:
...
class StuckDetector(private val windowSize: Int = 8) {
    ...
    private fun checkSameAction(): Signal? {
        // Threshold 4 to avoid false positives on legitimate repeated actions
        // (e.g. scroll in a long list, multiple taps in a form).
        if (actions.size < 4) return null
        val last4 = actions.toList().takeLast(4)
        return if (last4.all { it == last4[0] }) {
            Signal.SameAction(last4[0].take(50), 4)
        } else null
    }
    ...
    private fun checkHighRepetition(): Signal? {
        ...
        // Threshold 4 to match same-action threshold
        return if (maxEntry.value >= 4) {
