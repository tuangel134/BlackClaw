package com.blackclaw.android.support
...
object DebugReportManager {
    ...
    fun buildReport(context: Context): File {
        ...
        val output = File(reportDir, "blackclaw-debug-$timestamp.zip")
    ...
    private fun buildSummary(context: Context): String {
        ...
        return buildString {
            appendLine("BlackClaw Debug Report")
            ...
            val automationProfiles = com.blackclaw.android.automation.AutomationProfileStore.list()
            appendLine("- Automation profiles: ${automationProfiles.size} (${automationProfiles.count { it.enabled }} enabled)")
            appendLine("- Legacy automation rules: ${com.blackclaw.android.automation.AutomationRuleStore.list().size}")
            appendLine()
            appendLine("LLM")
            appendLine("- Selected mode: ${config.activeMode}")
            appendLine("- Effective mode: ${config.effectiveMode}")
    ...
    private fun buildBugReportTemplate(context: Context): String {
        return buildString {
            appendLine("BlackClaw Bug Report Template")
            appendLine()
            ...
            appendLine("adb logcat -v time > blackclaw-logcat.txt")
            appendLine("# reproduce once, then stop with Ctrl+C")
            appendLine("adb shell dumpsys activity top > blackclaw-activity-top.txt")
            appendLine("adb shell dumpsys activity services com.blackclaw.android > blackclaw-services.txt")
            appendLine()
            appendLine("Open a new GitHub issue: https://github.com/tuangel134/BlackClaw")
