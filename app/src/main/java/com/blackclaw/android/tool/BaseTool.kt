...
abstract class BaseTool {
    ...
    companion object {
        ...
        val WAIT_AFTER_PARAM = ToolParameter(
            "wait_after",
            "integer",
            "Optional ms to wait after this action (e.g. 2000 for page load). Default 0.",
            false
        )
    }
    ...
    /**
     * One-line summary for the compact tool catalog injected into the system
     * prompt (progressive disclosure). Default: first sentence of the English
     * description, capped. Tools may override for a tighter blurb.
     *
     * Keeping this short matters: the catalog lists ALL ~85 tools, so every
     * extra word here is multiplied across the whole registry.
     */
    open fun getBrief(): String {
        val d = getDescriptionEN().trim()
        // First sentence (up to a period) or first 90 chars, whichever is shorter.
        val firstSentence = d.substringBefore(". ").substringBefore(".\n")
        val brief = if (firstSentence.length in 1..100) firstSentence else d.take(90)
