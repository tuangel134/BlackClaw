# BlackClaw Skill File Specification v1.0
...
**Location:** `assets/skills/` (built-in), `/sdcard/BlackClaw/skills/` (user-created)
**Encoding:** UTF-8
...
tools: <required> Comma-separated list of BlackClaw tool identifiers this skill uses.
...
- `tools` must use exact BlackClaw tool identifiers. Unknown tools produce a load-time warning; the skill still loads with valid tools. Zero valid tools = skill skipped.
